...
void vkutil::generate_cubemap_mipmaps(VkCommandBuffer cmd, VkImage image, VkExtent2D imageSize, uint32_t mipLevels)
{
    if (mipLevels < 2) {
    ...
    for (uint32_t mip = 0; mip < mipLevels; mip++) {
        VkExtent2D halfSize = { mipSize.width / 2, mipSize.height / 2 };
        ...
        if (mip + 1 < mipLevels) {
            VkImageBlit2 blitRegion{ .sType = VK_STRUCTURE_TYPE_IMAGE_BLIT_2, .pNext = nullptr };
            blitRegion.srcOffsets[1] = { (int32_t)mipSize.width, (int32_t)mipSize.height, 1 };
